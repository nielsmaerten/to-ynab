...
function detectSource(file, options) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!detectedSource) {
      reject(
        new Error(
          `${file} - No matching source found. List of valid sources: [${Object.keys(
            sources
          )}]`
        )
      );
...
function headerMatchesSource(headerCells, sourceConfig) {
  return (
    sourceConfig.headers.length === headerCells.length &&
    sourceConfig.headers.every((h, i) => h == headerCells[i])
  );
}
...
function loadFile(file, options) {
  return new Promise((resolve, reject) => {
    ...
    if (options.csvstring && !file) {
      reject(Error("A valid csv string needs to be provided"));
      return;
    }
    ...
    if (!file) {
      reject(Error("A valid .csv file needs to be provided"));
      return;
    }

    //Validate csv file
    if (!/.*\.csv$/i.test(file)) {
      reject(Error("File provided is does not have a .csv extension"));
      return;
    }
