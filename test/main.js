...
describe("Main", () => {
    ...
    it("Should return error if a file is not passed", () => {
        return ynab_generator().catch(err => {
    ...
    it("Should return error if called a file without .csv is passed", () => {
        return ynab_generator("file.xls").catch(err => {
            assert.instanceOf(err, Error);
            assert.equal(
        err.message,
        "File provided is does not have a .csv extension"
      );
        });
    });

    it("Should return error if csv file is empty", () => {
        return ynab_generator(`./${config.testDir}/${config.testFiles[3]}`).catch(
      err => {
          assert.instanceOf(err, Error);
    ...
    it("Should return error if csv contains only header rows", () => {
        return ynab_generator(`./${config.testDir}/${config.testFiles[2]}`).catch(
      err => {
          assert.instanceOf(err, Error);
    ...
    it("Should generate a ynab.csv file without any options in process.cwd() folder", () => {
        return ynab_generator(`./${config.testDir}/${config.testFiles[0]}`).then(
      result => {
          assert.equal(result, "File ynab.csv written successfully!");
          assert.isOk(
          fs.existsSync(process.cwd() + "/ynab.csv"),
          "File was generated in root"
        );
      }
    );
    });
